...
/**
 * Emulation of SerialVGAMonitor from HobbyTronics
 * 
 * @author Xtase - fgalliat @Apr-2017
 *
 *         Still missing : <br/>
 *         cursor <br/>
...
public class VGATInterfaceFB {
  ...
  static boolean DO_ONLY_SINGLE_TEST = false;
	...
	protected static VGATInterfaceFB instance = null;

	protected static int TTY_WIDTH = 100;
	protected static int TTY_HEIGHT = 50;

	public static VGATInterfaceFB getInstance() {
		return instance;
	}

	public VGATInterfaceFB(TFTScreen dispSrv) {
		instance = this;
  ...
  static int HERE_MODIFIER = 1;
	...
	static int ftHeight = 10;
	static int ftWidth = 7 - HERE_MODIFIER;
  ...
  /*synchronized*/ void doRedraw() {
	...
	protected static class TextWindow {
      ...
      //static char BLANK_CHAR = 0x00;
        ...
        protected void setDirty(boolean propagate) {
			this.isDirty = true;
			// .... ????
			if (propagate) VGATInterfaceFB.getInstance().setDirty(this);
		}
		...
		public TextWindow(int id, int x, int y, int w, int h, boolean borders, String title) {
			...
			VGATInterfaceFB.getInstance().windows[id] = this;
		...
		protected void applyLineColor(VGATInterfaceFB iface, int y) {
		...
		public void render(VGATInterfaceFB iface) {
          VGATInterfaceFB.inRender = true;
			String line = "";
          ...
          VGATInterfaceFB.inRender = false;
		}
	...
	protected static class ColorLine {
		...
		public ColorLine(int y1, int y2, Color fg, Color bg) {
			...
			for (int y = y1; y <= y2; y++) {
				VGATInterfaceFB.getInstance().screenlines[y] = this;
			}

			// TODO : fewer times
			
          	// BEWARE I rempoved this code
            VGATInterfaceFB.getInstance().screenIsDirty = true;
          	VGATInterfaceFB.getInstance().setDirty();
          /*
          for (int i = 0; i < VGATInterfaceFB.getInstance().windows.length; i++) {
			TextWindow win = VGATInterfaceFB.getInstance().windows[i];
			if (win != null && win.isAttached()) {
    ...
    static int LOCAL_SCREEN_Y = -1;
     ...
     static protected void d_drawString(String text, int x, int y, int color, boolean renderSpaces) {
       dispSrv.blitString(text, TFTScreen.SCREEN_VIRTUAL_X+LOCAL_SCREEN_X+x, TFTScreen.SCREEN_VIRTUAL_Y+LOCAL_SCREEN_Y+y, color, renderSpaces);
       screenIsDirty = true;
     }
    
     static synchronized void d_blit() {
       //if ( !VGATInterfaceFB.inRender ) {
	...
	void vgat_focuswin(int id) {
		focus(id);
	}

	void _vga_cmd(String str, int timeWait) {
		submit("^[" + str);
	}

	void vgat_print(String str) {
		print(str);
	}

	void vgat_createwin(int id, int x, int y, int w, int h, boolean border, String title) {
	...
	public void test() {
		...
		// // +2 & +3 cf borders
		// vgat_createwin(1, 10 - 1, 10 - 5 - 2, 100 - 10 - 10 + 2, 50 - 10 - 10 +
		// 3, true, "Main Window");
		//
		// vgat_createwin(2, 10, 0, 100 - 10 - 10, 1, false, NULL);
		// _vga_cmd("l,0,1,222,101", 2); // White text on purple lines 0-1
		...
		// vgat_print("Xtase - fgalliat @ 01/2017");
		//
		// vgat_createwin(4, 10 - 1, 50 - 10 - 10 + 6 + 1, 100 - 10 - 10 + 2, 10 +
		// 1, true, "Status");
		// _vga_cmd("l,37,47,222,001", 2); // white text on blue lines 36-46
      ...
      //VGATInterfaceFB.getInstance().setDirty();
      //VGATInterfaceFB.getInstance().windows[1].setDirty();
	}
